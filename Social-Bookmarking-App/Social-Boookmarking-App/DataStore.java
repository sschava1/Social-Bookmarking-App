...
import com.shirish.thrillio.constants.BookGenre;
import com.shirish.thrillio.constants.Gender;
import com.shirish.thrillio.constants.MovieGenre;
...
import com.shirish.thrillio.util.IOUtil;
...
public class DataStore {
	...
	private static void loadUsers() {
		 ...
		 * "user0@semanticsquare.com", "test", "John", "M", Gender.MALE,
		 * UserType.USER); users[1] = UserManager.getInstance().createUser(1001,
		 * "user1@semanticsquare.com", "test", "Sam", "M", Gender.MALE,
		 * UserType.USER); users[2] = UserManager.getInstance().createUser(1002,
		 * "user2@semanticsquare.com", "test", "Anita", "M", Gender.MALE,
		 * UserType.EDITOR); users[3] =
		 * UserManager.getInstance().createUser(1003,
		 * "user3@semanticsquare.com", "test", "Sara", "M", Gender.FEMALE,
		 * UserType.EDITOR); users[4] =
		 * UserManager.getInstance().createUser(1004,
		...
		for (String row : data) {
			...
			int gender = (values[5].equals("m") ? Gender.MALE
					: (values[5].equals("f") ? Gender.FEMALE : Gender.TRANSGENDER));
	...
	private static void loadMovies() {
		 ...
		 * "Citizen Kane", "", 1941, new String[] { "Orson Welles",
		 * "Joseph Cotten" }, new String[] { "Orson Welles" },
		 * MovieGenre.CLASSICS, 8.5); bookmarks[1][1] =
		 * BookmarkManager.getInstance().createMovie(3001, "The Grapes of Wrath"
		 * , "", 1940, new String[] { "Henry Fonda", "Jane Darwell" }, new
		 * String[] { "John Ford" }, MovieGenre.CLASSICS, 8.2); bookmarks[1][2]
		 * = BookmarkManager.getInstance().createMovie(3002,
		 ...
		 * "The Big Bang Theory", "", 2007, new String[] { "Kaley Cuoco",
		 * "Jim Parsons" }, new String[] { "Chuck Lorre", "Bill Prady" },
		 * MovieGenre.TV_SHOWS, 8.7); bookmarks[1][4] =
		 * BookmarkManager.getInstance().createMovie(3004, "Ikiru", "", 1952,
		 * new String[] { "Takashi Shimura", "Minoru Chiaki" }, new String[] {
	...
	private static void loadBooks() {
		 ...
		 * "Henry David Thoreau" }, BookGenre.PHILOSOPHY, 4.3); bookmarks[2][1]
		...
		for (String row : data) {
			...
			bookmarks[1][colNum++] = BookmarkManager.getInstance().createBook(Long.parseLong(values[0]), values[1],
